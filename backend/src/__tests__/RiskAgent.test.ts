...
describe('RiskAgent', () => {
  ...
  it('should reject high risk regardless of APY', async () => {
    ...
    const trace: AgentTrace[] = [];
    const { result } = await agent.review(
      { protocol: 'Risky', apy: 3.0, riskLevel: 'high' },
      trace,
      1000
    );
    expect(result.decision).toBe('reject');
  ...
  it('should approve medium risk with APY below threshold', async () => {
    ...
    const trace: AgentTrace[] = [];
    const { result } = await agent.review(
      { protocol: 'MediumSafe', apy: 4.0, riskLevel: 'medium' },
      trace,
      1000
    );
    expect(result.decision).toBe('approve');
  ...
  it('should return confidence <= 0.95', async () => {
    const agent = new RiskAgent();
    const trace: AgentTrace[] = [];
    const { confidence } = await agent.review(
      { protocol: 'Aave', apy: 4.2, riskLevel: 'low' },
      trace,
      1000
    );
  ...
  it('should have lower confidence for rejections', async () => {
    const agent = new RiskAgent();
    const t1: AgentTrace[] = [];
    const { confidence: c1 } = await agent.review(
      { protocol: 'Aave', apy: 4.2, riskLevel: 'low' },
      t1,
      1000
    );
    const t2: AgentTrace[] = [];
    const { confidence: c2 } = await agent.review(
  ...
  it('should return AXL influence with approval ratio', async () => {
    const agent = new RiskAgent();
    const trace: AgentTrace[] = [];
    const { axlInfluence } = await agent.review(
      { protocol: 'Aave', apy: 4.2, riskLevel: 'low' },
      trace,
      1000
    );
