...
console.log(
  '[BOOT] ENS RPC:',
  process.env.ALCHEMY_MAINNET_RPC_URL ? 'configured' : 'not set (using public fallback)'
);
console.log('[BOOT] AXL base:', AXL_BASE_URL);
const llmStatus = process.env.OPENROUTER_API_KEY
  ? 'enabled (provider: OpenRouter)'
  : process.env.GROQ_API_KEY
    ? 'enabled (provider: Groq)'
    : 'disabled';
console.log('[BOOT] LLM:', llmStatus);
console.log(
  '[BOOT] Uniswap API:',
  process.env.UNISWAP_API_KEY ? 'enabled' : 'disabled (CoinGecko quote fallback active)'
);
console.log(
  '[BOOT] 0G memory:',
  process.env.ZEROG_MEMORY_KV_URL && process.env.ZEROG_MEMORY_LOG_URL ? 'enabled' : 'disabled'
);
